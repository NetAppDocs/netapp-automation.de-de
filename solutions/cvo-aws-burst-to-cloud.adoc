...
summary: Dieser Artikel unterstützt die NetApp Cloud Volumes ONTAP for AWS Automation Solution, die NetApp -Kunden über den NetApp Console Automation Hub zur Verfügung steht. 
---
...
.Bevor Sie beginnen
* Sie müssen die link:https://console.netapp.com/automationHub["Cloud Volumes ONTAP AWS – Burst in die Cloud"^] Automatisierungslösung über die Web-Benutzeroberfläche der Konsole. Die Lösung ist verpackt als `cvo_aws_flexcache.zip`Die
